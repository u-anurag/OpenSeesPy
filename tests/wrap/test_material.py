...
def test_bouc_wen():
    osi = opw.OpenseesInstance(dimensions=2)
    opw.uniaxial_material.BoucWen(osi, alpha=1.0, ko=1.0, n=1.0, gamma=1.0, beta=1.0, ao=1.0, delta_a=1.0, delta_nu=1.0, delta_eta=1.0)
...
def test_init_strain_material():
    osi = opw.OpenseesInstance(dimensions=2)
    bilinear_mat = opw.uniaxial_material.Steel01(osi, fy=1, e0=1, b=1)
    opw.uniaxial_material.InitStrainMaterial(osi, other=bilinear_mat, init_strain=1.0)
