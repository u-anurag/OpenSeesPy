...
class plot_deformedshape_Events:
    ...
    def __init__(self, Model="none", LoadCase="none", tstep = -1, scale = 10, overlap='no'):
        self.Model = Model
    ...
    def _plot_on_click(self, event):
        ...
        ind = event.ind[0]
        fig, ax = plt.subplots()
        # Disp = self.nodeArray[ind,1:], self.scale*Disp_nodeArray[:,ind,0]
        line = ax.plot(timeSteps, self.scale*Disp_nodeArray[:,ind,0])
        ax.set_xlabel('time (s)')
        ax.set_ylabel('Displacement')
        # print(ind)
        # print(DeflectedNodeCoordArray[ind,:])
        # points = tuple(zip(xdata[ind], ydata[ind]))
        # print('onpick points:', points)
    ...
    def plot(self):
        ...
        if len(nodecoords(nodetags[0])) == 2:
            ...
            ax.text2D(0.1, 0.90, printLine, transform=ax.transAxes)
        # plt.plot(DeflectedNodeCoordArray[:,0], DeflectedNodeCoordArray[:,1], DeflectedNodeCoordArray[:,2], 'o', picker=5)
        ipltf._setStandardViewport(fig, ax, DeflectedNodeCoordArray, len(nodecoords(nodetags[0])))					
        plt.axis('on')
